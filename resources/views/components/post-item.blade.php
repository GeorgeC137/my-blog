<article class="flex bg-white flex-col shadow my-4">
    <!-- Article Image -->
    <a href="{{ route('view', $post) }}" class="hover:opacity-75">
        <img src="{{ $post->getThumbnail() }}" alt="{{ $post->title }}" class="object-contain aspect-[4/3]">
    </a>
    <div class="bg-white flex flex-col justify-start p-6">
        ...
        @if ($showAuthor)
            <p href="#" class="text-sm pb-3">
                ...
                {{ $post->getFormattedDate() }} | {{ $post->humanReadTime }}
            </p>
        @endif
