...
<aside class="w-full md:w-1/3 flex flex-col items-center px-3">
    ...
    <div class="w-full bg-white shadow flex flex-col my-4 p-6">
        ...
        @foreach ($categories as $category)
            <a href="{{ route('by-category', $category) }}" class="rounded py-2 px-3 text-semibold block {{ request('category')?->slug === $category->slug ? 'bg-blue-600 text-white' : '' }}">
    ...
    <div class="w-full bg-white shadow flex flex-col my-4 p-6">
        <p class="text-xl font-semibold pb-5">
            {{ \App\Models\TextWidget::getTitle('my-career-path') }}
        </p>
        <p class="pb-2">
            {!! \App\Models\TextWidget::getContent('my-career-path') !!}
        </p>
